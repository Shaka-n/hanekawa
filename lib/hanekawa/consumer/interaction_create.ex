defmodule Hanekawa.Consumer.InteractionCreate do
  ...
  def do_movie_night(%Interaction{data: %{options: [%{options: [%{value: date}, %{value: movie_title}]}]}} = interaction, :schedule) do
    ...
    case MovieNights.schedule_movie_night(%{date: date, movie_title: movie_title, creator_id: creator_id}) do
      {:ok, movie_night} ->
        response = message_response("Movie night scheduled! We're watching #{movie_night.movie_title} on #{MovieNights.date_to_mdy_string(movie_night.date)}!")
  ...
  def do_movie_night(%Interaction{data: %{options: [%{options: [%{value: date}]}]}} = interaction, :schedule) do
    creator_id = Integer.to_string(interaction.user.id)
    case MovieNights.schedule_movie_night(%{date: date, creator_id: creator_id}) do
      {:ok, movie_night} ->
        response = message_response("Movie night scheduled! We're watching #{movie_night.movie_title} on #{MovieNights.date_to_mdy_string(movie_night.date)}!")
        Api.create_interaction_response(interaction.id, interaction.token, response)
      {:error, _error} ->
  ...
  def do_movie_night(interaction, :next) do
    case MovieNights.get_next_movie_night() do
      nil ->
        ...
        Api.create_interaction_response(interaction.id, interaction.token, response)
      movie_night ->
        response = message_response("The next movie night is scheduled for #{MovieNights.date_to_mdy_string(movie_night.date)}. We decided on #{movie_night.movie_title}")
        Api.create_interaction_response(interaction.id, interaction.token, response)
    end
  end

  def do_movie_night(%Interaction{data: %{options: [%{options: [%{value: date}, %{value: new_date}]}]}} = interaction, :reschedule) do
    case MovieNights.reschedule_movie_night(%{date: date, new_date: new_date}) do
      {:ok, movie_night} ->
        response = message_response("Movie night scheduled! We're watching #{movie_night.movie_title} on #{movie_night.date}! ")
        Api.create_interaction_response(interaction.id, interaction.token, response)
        {:error, _error} ->
  ...
  def do_movie_night(%Interaction{data: %{options: [%{options: [%{value: date}, %{value: new_date}, %{value: movie_title}]}]}} = interaction, :reschedule) do

    case MovieNights.reschedule_movie_night(%{date: date, new_date: new_date, movie_title: movie_title}) do
      {:ok, movie_night} ->
        response = message_response("Movie night scheduled! We're watching #{movie_night.movie_title} on #{MovieNights.date_to_mdy_string(movie_night.date)}! ")
        Api.create_interaction_response(interaction.id, interaction.token, response)
        {:error, _error} ->
  ...
  def do_movie_night(%Interaction{data: %{options: [%{options: [%{value: date}]}]}} = interaction, :cancel) do
    case MovieNights.cancel_movie_night(date) do
      {:ok, movie_night} ->
        # date = Date.to_string(movie_night.date)
        response = message_response("The movie night on " <> MovieNights.date_to_mdy_string(movie_night.date) <> " has been canceled.")
        Api.create_interaction_response(interaction.id, interaction.token, response)
        {:error, _error} ->
